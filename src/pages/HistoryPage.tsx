  ...
  return (
    <div className="min-h-dvh overflow-clip isolate relative bg-bg pb-28">
      ...
      {mode === 'calendar' ? (
        ...
        <TimelineView days={days} onMore={() => setDays((d) => d + PAGE_DAYS)} />
      )}
    </div>
  )
}
...
function TimelineView({ days, onMore }: { days: number; onMore: () => void }) {
  const navigate = useNavigate()
  const end = todayStr()
  const start = addDays(end, -(days - 1))

  const rows = useLiveQuery(async (): Promise<TimelineRow[]> => {
    const [sessions, rests, sets, wes] = await Promise.all([
    ...
    return [...sessionRows, ...restRows].sort((a, b) =>
